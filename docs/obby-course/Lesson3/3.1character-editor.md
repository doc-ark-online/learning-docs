...
![learn_3_18](https://arkimg.ark.online/learn_3_18.webp)

<p align="center">（图 16 运行时看到服装需要这样设置）</p>

图 16 步骤：①找到对象管理器 -->②选中Player -->③找到属性面板 -->④**取消勾选"使用平台角色形象选项"**

### 头盔插槽部分想要游戏中显示

虽然编辑器中我们只做了头盔，但是实际游戏运行起来我们是看不见头盔的，还需要一部分代码支持

![learn_3_19](https://arkimg.ark.online/learn_3_19.webp)
图 17 步骤：①**工程内容**面板选中**脚本**，点击**新建脚本** -->②将新建的脚本**改名helmeted** -->③将helmeted脚本拖拽到**对象管理器**中的**对象**节点下

helmeted代码中添加如下内容：
```ts

@Core.Class
export default class helmeted extends Core.Script {
	...
	// 客户端执行相关代码
	    ...
	    // 等待角色appearance准备完成
            await this.Character.appearanceReady()
                
            let humanoidV2 = this.Character.setAppearance(Gameplay.HumanoidV2)
                
            // 鸭子骑士的guid
            humanoidV2.setAppearanceData(["9F11B64F41A2EE5DEDA287A75AD075F4"], () => {

                // 不调用这一行，其他玩家将看不到自己换装
                humanoidV2.appearanceSync()
                // 设置鸭子骑士的插槽数据
                humanoidV2.setSlot("9F11B64F41A2EE5DEDA287A75AD075F4", true)
                // 设置头盔
...
代码中的**9F11B64F41A2EE5DEDA287A75AD075F4**为新保存的角色的**工程内容ID**
