...
我们根据之前 [2.2 客户端与服务端](https://learning.ark.online/md/2.2.html) 的教程部分，了解到在游戏开发中，有些代码是需要在客户端编写，有些代码需要在服务端编写。那我们想象一个开发场景，在场景中有一个特效，特效播放完成后，游戏结束并保存游戏数据。这个开发场景非常简单，但是要注意的是播放特效的代码应该在客户端执行，而游戏结束保存数据的代码是服务端执行，这时候这个开发场景就要使用客户端代码来调用服务端代码了。
