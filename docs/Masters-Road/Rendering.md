...
如下视频，打开PC控制台后，这里输入命令 **Stat UNIT** 打开了性能面板
...
> 常见的优化手段有：
>
> 1. 使用单一对象达成拼装的效果（降低顶点数、减少 Draws 次数）
> 2. 复用模型（减少 Draws）
> 3. 注意模型动静态（减少 Draws）
...
1. 如下视频，一开始就放在场景里的相同的对象，游戏运行后，它们处于**静态状态**，他们的 Draws 会得到**合批优化**。
   ...
   通过**代码动态创建的相同模型**也不会被优化，它的 Draws 是单算的，看向多个相同模型 Draws 就会上升了。
...
2. 如下视频，在主视口中，移动模型的远近，整个移动过程包括**两次 Prims 的值变动**。
   ...
   由此可知，**对象被裁剪后就不会被渲染了，能节省 Draws 和Prims**。

**LOD（多细节层次 既 Level Of Details，用于降低非重要物体的面数和细节数，提高渲染效率）**
...
4. 如下图，在资源库中找到模型，在红点1位置点击+号，会弹出右侧红点2里的资源详情，在红点3处能确认到该资源的顶点数，顶点数理论上是有多少模型，就会上涨多少，当前在游戏中看到的 Prims 参数由于合批，其值可能是比真实数据偏低的。

![image-20231218110133200](https://arkimg.ark.online/image-20231218110133200.png)
...
> 2. 对象被裁剪后，其不会造成渲染消耗
> 3. 随着对象的远近变化，LOD 相关优化将使得对象顶点数下降
...
![image-20231215160928355](https://arkimg.ark.online/image-20231215160928355-1702812850842-3.png)

![image-20231215161414983](https://arkimg.ark.online/image-20231215161414983-1702628088764-1.png)

## 使用多个相同对象拼装制作场景

**案例2**，如下图，一个项目用大量方块拼出不规则地面![image-20231208171930304](https://arkimg.ark.online/image-20231208171930304.png)

打开项目去除周边建筑后，实际看到地面的效果如下图，中间镂空部分是用来放泳池的，如果使用一整块地板，中间泳池部分和周边不规则部分就很不好处理，用案例1的方案可能效果就会大打折扣

![image-20231208171459611](https://arkimg.ark.online/image-20231208171459611-1702030293928-1.png)
...
![image-20231218111021715](https://arkimg.ark.online/image-20231218111021715.png)

实际上这些椰子树模型目前一共就只有3种，通过少量模型，调整他们的大小、旋转、缩放，能摆出非常多的种类搭配，营造出想要的场景。

![image-20231218111057988](https://arkimg.ark.online/image-20231218111057988.png)
...
![image-20231206104557722](https://arkimg.ark.online/image-20231206104557722.png)

比如这里将这个猫咪雕塑的裁剪距离设置为了1000

![image-20231206104623196](https://arkimg.ark.online/image-20231206104623196.png)

如下视频可以看到，我们走到了1000的距离才能看到这个猫咪，而其它远处的模型并没有被裁剪。
...
如下图，远处需要摆放一些树木作为装饰，但是放这种3700多顶点的树在远处，其实是不慎理想的

![image-20231218115327572](https://arkimg.ark.online/image-20231218115327572.png)

于是选择了如下图中的面片树，顶点数只有4

![image-20231218120226003](https://arkimg.ark.online/image-20231218120226003.png)

**案例2**

如下图从远处看过去，有一些建筑的顶部露在外边

![image-20231218120515928](https://arkimg.ark.online/image-20231218120515928.png)

对于远处玩家永远不会到达的地方，只要确定不会穿帮，就可以如下图只有一个塔尖

![image-20231218120625300](https://arkimg.ark.online/image-20231218120625300.png)

**案例3**

如下图，有的建筑可以允许进入室内，从门口我们就能看到内部的布置

![image-20231218121119652](https://arkimg.ark.online/image-20231218121119652.png)

也有如下图这样的建筑，没有进入的地方

![image-20231218121228658](https://arkimg.ark.online/image-20231218121228658.png)

在主视口中移动到建筑内部，也能看到，里边是什么都没有的

![image-20231218121250405](https://arkimg.ark.online/image-20231218121250405.png)
...
人型对象包含大量材质，放那不动对渲染的效率降低都是很多的，也许在电脑上编辑器里运行还行，手机上性能就会巨大下降。
...
![image-20231205185831645](https://arkimg.ark.online/image-20231205185831645.png)
