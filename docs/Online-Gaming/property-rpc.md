...
在上面的示例中，对于继承了 `Script` 的脚本中的replicated属性，在服务端修改时，会同步到每一个客户端并触发回调函数。如果存在一些只有玩家自身才会关注的属性，比如玩家自身的闯关倒计时，就不需要同步到所有客户端。这时，可以使用另一种方法单独同步属性给对应玩家。

指定客户端与同步到所有客户端的replicated只有一点不同，指定客户端的replicated需要在继承 `PlayerState` 的脚本里生效。
...
在每个客户端只会为这个客户端的player创建一份 `PlayerState` 实例，所以在客户端无法获取其他玩家的 `PlayerState` 实例。
